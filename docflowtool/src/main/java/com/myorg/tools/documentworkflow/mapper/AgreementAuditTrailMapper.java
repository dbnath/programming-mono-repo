...
public class AgreementAuditTrailMapper implements RowMapper<DocWkflwProcess> {
	...
	public DocWkflwProcess mapRow(ResultSet rs, int rowNum) throws SQLException {
		DocWkflwProcess docWorkflow = new DocWkflwProcess();
	      if (! DocumentWorkflowToolUtility.isEmpty(rs)) {
				...
				docWorkflow.setVersionId(rs.getInt("ID_VER"));
				...
				docWorkflow.setRoleId(rs.getInt("ID_ROLE_AS"));
				docWorkflow.setStatusCode(rs.getInt("ID_WF_STATUS"));
				docWorkflow.setStatusDescription(rs.getString("TX_WF_STATUS"));
				docWorkflow.setComments(DocumentWorkflowToolUtility.isEmptyValue(rs.getString("COMMENTS"))?"":rs.getString("COMMENTS"));
			    docWorkflow.setCreatedBy(rs.getString("CREATED_BY"));
			    docWorkflow.setCreationDate(rs.getTimestamp("CREATED_DT"));
