...
landingCtrl.$inject = ['$stateParams','service','$scope','$rootScope','$templateCache','$log','uiGridConstants',"$filter","ngDialog"];                           
                              
    function landingCtrl ($stateParams,service,$scope,$rootScope,$templateCache,$log,uiGridConstants,$filter,ngDialog){
...
//		    else
    ...
    '</div>';
	  $scope.statusTypes = [{
	      ...
	      ID: 2,
	      type: 'Maker -In Progress'
	    }, {
	      ID: 3,
	      type: 'Maker-Complete'
	    }, {
	      ID: 4,
	      type: 'Hold-Document-IIIeligible'
	    }, 
	    {
		      ID: 5,
		      type: 'Hold-Wrong-Document'
		    },
		    {
			      ID: 6,
			      type: 'Query Hold'
	...
	$scope.gridOptionsmylist = {
		...
		//	data: 'users',
			columnDefs: [
			             ...
			             {field: "docId", displayName: "Agreement Id", enableCellEdit: false},
			           
			             {field: "wfAssignmentGroupName", displayName: "LOB", enableCellEdit: false
			            	},
			            	 {field: "assignedTo", displayName: "Agreement Type", enableCellEdit: false},
			            	 {field: "status",
			            		   displayName: 'Status',
			            	        editType: 'dropdown',
			            	        enableCellEdit: true,
			            	        editableCellTemplate: 'ui-grid/dropdownEditor',
			            	        editDropdownOptionsArray: $scope.statusTypes,
...
//			    }
	...
	$scope.gridOptionsmylist.onRegisterApi = function(gridApi){
		  ...
		  gridApi.edit.on.afterCellEdit($scope, function(rowEntity, colDef) {
		      alert(rowEntity.docId + ' ' + rowEntity.status);
		    });
